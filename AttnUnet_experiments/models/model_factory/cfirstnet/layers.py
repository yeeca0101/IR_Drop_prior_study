...
from timm.layers import LayerNorm2d, GELU
...
class AddCoords(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        if self.rank == 1:
            ...
            xx_channel = xx_channel.to(input_tensor.device)
            yy_channel = yy_channel.to(input_tensor.device)
            zz_channel = zz_channel.to(input_tensor.device)
...
class Conv2dNormAct(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding=0, dw_conv=False,
                 stride=1, act_layer=GELU, norm_layer=LayerNorm2d):
        super().__init__()
        groups = out_channels if dw_conv else 1
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, bias=True, groups=groups)
