...
class AddCoords(nn.Module):
    def __init__(self, rank, w=256,h=256,with_r=False, use_cuda=True,skiptile=False,device='cpu'):
    ...
    def forward(self, input_tensor):
        ...
        if self.rank == 1:
            ...
            if torch.cuda.is_available and self.use_cuda:
                ...
                xx_channel = xx_channel.cuda(self.device)
            ...
            if torch.cuda.is_available and self.use_cuda:
                input_tensor = input_tensor.cuda(self.device)
                xx_channel = xx_channel.cuda(self.device)
                yy_channel = yy_channel.cuda(self.device)
            ...
            if torch.cuda.is_available and self.use_cuda:
                ...
                xx_channel = xx_channel.cuda(self.device)
                yy_channel = yy_channel.cuda(self.device)
                zz_channel = zz_channel.cuda(self.device)
...
class CoordConv1d(conv.Conv1d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, with_r=False, use_cuda=True,device='cpu'):
        super(CoordConv1d, self).__init__(in_channels, out_channels, kernel_size,
                                          stride, padding, dilation, groups, bias)
        self.rank = 1
        self.addcoords = AddCoords(self.rank, with_r, use_cuda=use_cuda,device=device)
...
class CoordConv2d(conv.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, with_r=False, use_cuda=True,skiptile=True,device='cpu'):
        super(CoordConv2d, self).__init__(in_channels, out_channels, kernel_size,
                                          stride, padding, dilation, groups, bias)
        self.rank = 2
        self.addcoords = AddCoords(rank=self.rank, with_r=with_r, use_cuda=use_cuda,skiptile=skiptile,device=device)
